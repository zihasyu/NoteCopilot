            ...
            dedent("""
                ...
                **1. 'respond' - 信息充足，立即生成最终响应** 【最高优先级】
                   - 使用场景：
                     ...
                     * 已完成博客发布流程
                     * 已执行步骤 >= 3 且获取了关键信息
                   - 对于笔记管理任务，一旦核心目标达成就应该立即 respond

                **2. 'continue' - 当前计划合理，继续执行** 【次优先级】
                   - 使用场景：剩余步骤对完成任务必要
                   - 例如：检索后需要分析、分析后需要发布

                **3. 'replan' - 当前计划需要调整** 【最低优先级】
                   - 使用场景：遗漏关键步骤或计划顺序不合理
                   - ⚠️ **限制**：
                     * 新步骤数量必须 <= 当前剩余步骤数
                     * 总步骤数已执行 >= 5 次时，禁止 replan

                笔记任务评估标准：
                - 检索任务：找到相关笔记即可 respond
                - 增强任务：完成格式优化或摘要生成即可 respond
                - 发布任务：成功发布博客即可 respond
