...
class DocumentSplitterService:
    ...
    def split_markdown(self, content: str, file_path: str = "") -> List[Document]:
        ...
        if not content or not content.strip():
            print(f"Markdown 文档内容为空: {file_path}")
            return []
        ...
        try:
            ...
            print(f"Markdown 分割完成: {file_path} -> {len(final_docs)} 个分片")
            return final_docs

        except Exception as e:
            print(f"Markdown 分割失败: {file_path}, 错误: {e}")
            raise
    ...
    def split_text(self, content: str, file_path: str = "") -> List[Document]:
        ...
        try:
            ...
            print(f"文本分割完成: {file_path} -> {len(docs)} 个分片")
            return docs

        except Exception as e:
            print(f"文本分割失败: {file_path}, 错误: {e}")
            raise
